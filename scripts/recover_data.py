...
def recovery_data(idx):
    name = ("_%0" + str(num_zero_padding) + 'd.npy')%(idx)
    if np.min(images[idx]) < -1:
        idx_list.append(idx)
        print(idx, end=' ')
        pass
    else:
    ...
    with open(os.path.join(train_data_dir, 'image' + name), 'wb') as f:
        np.save(f, -images[idx])
        
    with open(os.path.join(train_data_dir, 'masked_image' + name), 'wb') as f:
        np.save(f, -masked_images[idx])
        

num_cores = multiprocessing.cpu_count()
parmap.map(recovery_data, range(len(flipped_images)), pm_pbar={'desc': 'Saving flipped data'}, pm_processes=num_cores, pm_chunksize=num_cores)