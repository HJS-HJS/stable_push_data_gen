...
class ContactPointSampler(object):
    ...
    Raise:
        ValueErrror: number of available contact points is less than desired push directions
    ...
    def __init__(self, camera_intr, camera_extr, gripper_width=0.08, num_push_dirs=8):
        ...
        self._width_error_threshold = 0.002
    ...
    def get_contact_points(self, edge_list_uv, edge_list_xyz):
        ...
        distances = np.where(distances > self.gripper_width, 0, distances)
        distances = np.where(distances < (self.gripper_width - self._width_error_threshold), 0, distances)
        distances = np.where((distances - np.roll(distances, shift=1)) < 0 , 0, distances)
        distances = np.where((distances - np.roll(distances, shift=-1)) < 0 , 0, distances)
            
        # contact_pair_idx = np.where(np.abs(distances - self.gripper_width) <= self._width_error_threshold)
        # contact_pair_idx = np.where(np.triu(distances) != 0)
        contact_pair_idx = np.where(distances != 0)
